...
Kernels may have shape `(1 or B, 1 or C, kh, kw)`: share a kernel across samples
or channels, or supply separate per-sample/per-channel kernels. The Python
reference, differentiable C++ paths and fused CUDA paths all use the same
broadcast rules. A tensor generated by a kernel-prediction network retains its
gradient connection during training.
...
### Combined USRNet path

The residual simplification is also used by `ConvReverseDataNet`, which now
dispatches to the same v2–v7 backends instead of maintaining a separate FFT solve.
`ConverseUSRNet` accepts `backend` and `variant` arguments for both its data and
restoration blocks. Existing checkpoint parameter names remain unchanged.

```python
from models.converse_usrnet import ConverseUSRNet

model = ConverseUSRNet(backend="cuda", variant="v7").cuda().eval()
# Load the existing state_dict as usual; x and k must be on the model's device.
with torch.inference_mode():
    out = model(x, k, sf=2)
```

Use `backend="pytorch"` for the stable full-FFT reference, `variant="v2"` for
the full-FFT C++ baseline, and `variant="v7"` for real FFT plus fused inference.
The old zero-insertion upsampler is eliminated by the residual formula. Runtime
v3–v6 names refer to shared corrected code, not six sequential optimizations.
Rebuild the extension after updating to this version to enable batched kernels.
