...
#ifdef CONVERSE2D_USE_CUDA_KERNELS
...
static inline Tensor sfold_upsample_zero_insertion_autograd(const Tensor &x, int64_t s) {
...
static inline Tensor block_mean_autograd(const Tensor &input, int64_t s) {
    if (s == 1) return input;
    return at::avg_pool2d(input, /*kernel_size=*/{s, s}, /*stride=*/{s, s},
...
#else
    return sfold_upsample_zero_insertion_autograd(x, s);
#endif
}

static inline Tensor block_mean(const Tensor &input, int64_t s) {
#ifdef CONVERSE2D_USE_CUDA_KERNELS
    if (s == 1) return input;
    return block_mean_cuda(input, s);
#else
    return block_mean_autograd(input, s);
#endif
}
...
Tensor converse2d_forward(Tensor x, Tensor x0, Tensor weight, Tensor bias, int64_t scale, double eps) {
    ...
    Tensor F_STy       = at::fft_rfft2(STy, c10::nullopt, {-2, -1}, c10::nullopt);
    Tensor F_lambda_x0 = at::fft_rfft2(lambda_ * x0, c10::nullopt, {-2, -1}, c10::nullopt);

    Tensor FBFy = FBC * F_STy;
    Tensor FR   = FBFy + F_lambda_x0;

    Tensor x1       = FB * FR;
    Tensor x1_real  = at::fft_irfft2(x1, {Hs, Ws}, {-2, -1}, c10::nullopt);
    Tensor F2B_real = at::fft_irfft2(F2B, {Hs, Ws}, {-2, -1}, c10::nullopt);

    Tensor FBR  = block_mean(x1_real,  scale);
    Tensor invW = block_mean(F2B_real, scale);
    ...
    Tensor invWBR_exp = invWBR.view({B, C, H, 1, W, 1})
                        ...
                        .reshape({B, C, Hs, Ws});
    Tensor FCBinvWBR = FBC * at::fft_rfft2(invWBR_exp, c10::nullopt, {-2, -1}, c10::nullopt);

    Tensor FX  = (FR - FCBinvWBR) / lambda_;
    Tensor out = at::fft_irfft2(FX, {Hs, Ws}, {-2, -1}, c10::nullopt);
    return out;
}
