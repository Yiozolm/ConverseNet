...
// -------- accumulator mapping (keep numerics stable) ----------
template <typename T>
struct acc_type_map
{
    using type = float;
...
__global__ void weighted_block_mean_kernel(
    const scalar_t *__restrict__ in, // (B,C,Hs,Ws_r)
    scalar_t *__restrict__ out,      // (B,C,Ho,Wo_r)
    ...
    int Ws_full, long long total_out)
{
    ...
    // average over an s x s block with DC/Nyquist half-weights along last rFFT dim
...
__global__ void weighted_block_mean_grad_kernel(
    const scalar_t *__restrict__ grad_out, // (B,C,Ho,Wo_r)
    scalar_t *__restrict__ grad_in,        // (B,C,Hs,Ws_r)
    int B, int C, int Ho, int Wo_r, int s, int Hs, int Ws_r,
    int Ws_full, long long total_in)
{
    ...
    // denominator over that output block
    float denom = 0.0f;
    for (int di = 0; di < s; ++di)
    {
