...
template<>__device__ double add_rn(double a,double b){return __dadd_rn(a,b);}
template<class T>__device__ T mul_rn(T a,T b);
template<>__device__ float mul_rn(float a,float b){return __fmul_rn(a,b);}
template<>__device__ double mul_rn(double a,double b){return __dmul_rn(a,b);}
...
template<>__device__ double fma_rn(double a,double b,double c){return __fma_rn(a,b,c);}
...
template<>__device__ double norm(Z<double>z){return __dadd_rn(__dmul_rn(z.real(),z.real()),__dmul_rn(z.imag(),z.imag()));}
...
std::vector<Tensor> full_adjoint_output_cuda(Tensor g0,Tensor k0,Tensor q0,I s){
 ...
 AT_DISPATCH_FLOATING_TYPES(at::toRealValueType(g.scalar_type()),"full_adj_output",[&]{adj_output<scalar_t><<<(g.numel()+255)/256,256,0,stream>>>(g.data_ptr<Z<scalar_t>>(),k.data_ptr<Z<scalar_t>>(),q.data_ptr<Z<scalar_t>>(),t.data_ptr<Z<scalar_t>>(),d.data_ptr<Z<scalar_t>>(),g.numel(),g.size(1),q.size(2),q.size(3),s,k.size(0),k.size(1));});C10_CUDA_KERNEL_LAUNCH_CHECK();return {t,d};
}
std::vector<Tensor> full_adjoint_div_cuda(Tensor t0,Tensor q0,Tensor d0){
 ...
 AT_DISPATCH_FLOATING_TYPES(d.scalar_type(),"full_adj_div",[&]{adj_div<scalar_t><<<(t.numel()+255)/256,256,0,stream>>>(t.data_ptr<Z<scalar_t>>(),q.data_ptr<Z<scalar_t>>(),d.data_ptr<scalar_t>(),gy.data_ptr<Z<scalar_t>>(),gd.data_ptr<Z<scalar_t>>(),t.numel(),t.size(2)*t.size(3),t.size(1),d.size(0));});C10_CUDA_KERNEL_LAUNCH_CHECK();return {gy,gd};
}
std::vector<Tensor> full_adjoint_prediction_cuda(Tensor g0,Tensor p0,Tensor k0,Tensor gm0,I s,bool shared){
 ...
 AT_DISPATCH_FLOATING_TYPES(at::toRealValueType(g.scalar_type()),"full_adj_pred",[&]{adj_prediction<scalar_t><<<(p.numel()+255)/256,256,0,stream>>>(g.data_ptr<Z<scalar_t>>(),p.data_ptr<Z<scalar_t>>(),k.data_ptr<Z<scalar_t>>(),gm.data_ptr<Z<scalar_t>>(),gp.data_ptr<Z<scalar_t>>(),gk.data_ptr<Z<scalar_t>>(),p.numel(),p.size(1),gm.size(2),gm.size(3),s,k.size(0),k.size(1),shared);});C10_CUDA_KERNEL_LAUNCH_CHECK();return {gp,gk};
}
Tensor full_adjoint_kernel_cuda(Tensor k0,Tensor a0,Tensor b0,Tensor power0,I s){
 ...
 AT_DISPATCH_FLOATING_TYPES(power.scalar_type(),"full_adj_kernel",[&]{adj_kernel<scalar_t><<<(k.numel()+255)/256,256,0,stream>>>(k.data_ptr<Z<scalar_t>>(),a.data_ptr<Z<scalar_t>>(),b.data_ptr<Z<scalar_t>>(),power.data_ptr<scalar_t>(),out.data_ptr<Z<scalar_t>>(),k.numel(),power.size(2),power.size(3),s);});C10_CUDA_KERNEL_LAUNCH_CHECK();return out;
}
