...
static inline Tensor sfold_upsample_zero_insertion(const Tensor& x, int64_t s) {
  ...
  z.index_put_(
    {at::indexing::Slice(), at::indexing::Slice(),
     ...
     at::indexing::Slice(0, z.size(-1), s)}, x);
  return z;
}
...
static inline Tensor splits_mean_then_mean(const Tensor& a, int64_t s) {
  ...
  for (int64_t i = 0; i < L-2; ++i) view_shape.push_back(sizes[i]);
  ...
  for (int64_t i = 0; i < L-2; ++i) perm.push_back(i);
  ...
  for (int64_t i = 0; i < L-2; ++i) merge_shape.push_back(p.size(i));
