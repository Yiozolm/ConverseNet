"""Render the validation report from the checked-in machine measurements."""
import json
...
def matching(rows, row, variant):
    return next(r for r in rows if r["variant"] == variant and
                all(r[k] == row[k] for k in ("B", "C", "H", "W", "scale")))


def main():
    gpu, cpu = read("correctness_cuda.json"), read("correctness_cpu.json")
    inference, training = read("gpu_benchmark.json"), read("gpu_training_benchmark.json")
    smoke = read("pretrained_smoke.json")
    assert gpu["passed"] and cpu["passed"]
    text = ["# Converse2D 修复与 GPU 验证报告", "",
            ...
            "## 环境与验证", "",
            f"GPU：{inference['gpu']}；驱动 616.92；PyTorch {inference['torch']}；CUDA Toolkit 13.0；Python 3.12；MSVC 14.44；Nsight Systems 2026.1.2 / Compute 2026.1.0。", "",
            f"GPU 回归：{gpu['tests']} 项测试通过，包含 {gpu['forward_cases']} 组前向子用例和 {gpu['gradient_cases']} 组四类梯度对照。CPU-only 编译后运行同一套测试通过，CUDA 流测试跳过。", "",
            ...
            "|---|---:|---:|",
            f"| float64 | {max(gpu['max_abs'][f'torch.float64/v{i}'] for i in range(3,7)):.3e} | {gpu['max_abs']['torch.float64/v7']:.3e} |",
            f"| float32 | {max(gpu['max_abs'][f'torch.float32/v{i}'] for i in range(3,7)):.3e} | {gpu['max_abs']['torch.float32/v7']:.3e} |", "",
            ...
            "|---|---:|---:|---:|---:|---:|"]
    rows = inference["results"]
    for r in rows:
        if r["variant"] != "v7": continue
        previous = matching(rows,r,"legacy")
        vals = [matching(rows,r,v)["gpu_ms_median"] for v in ("legacy","v2","v6","v7")]
        text.append(f"| {r['C']}×{r['H']}×{r['W']} / {r['scale']} | " +
                    " | ".join(f"{v:.4f}" for v in vals) + f" | {previous['gpu_ms_median']/r['gpu_ms_median']:.2f}× |")
    large = next(r for r in rows if r["variant"] == "v7" and r["C"] == 64 and r["scale"] == 2)
    old = matching(rows,large,"legacy")
    text += ["", "v7 在这五个样例中比旧 main 快约 2.1–11.2 倍。小尺寸 scale=1 样例中 v6 比 v7 更快，实际选型仍应按工作负载测速。", "",
             f"C=64、256×256、scale=2 的增量峰值 allocated 显存由约 {old['peak_extra_bytes']/2**20:.1f} MiB 降至 {large['peak_extra_bytes']/2**20:.1f} MiB。此项不等于整进程总显存或 CUDA allocator 的 reserved 显存。", "",
             "同一组大尺寸测试对 float64 参考的最大绝对误差：", "",
             "| C×H×W / scale | legacy | v7 |", "|---|---:|---:|"]
    for r in rows:
        if r["variant"] == "v7":
            previous = matching(rows,r,"legacy")
            text.append(f"| {r['C']}×{r['H']}×{r['W']} / {r['scale']} | {previous['max_abs_error']:.3e} | {r['max_abs_error']:.3e} |")
    text += ["", "scale=1 的大图仍可有约 1e-4 的 float32 绝对误差，说明稳定公式改善了误差，但不会消除 FFT 和病态频率位置的全部舍入影响。", "",
             "## 训练性能", "",
    ...
    for r in rows:
        if r["variant"] == "v7":
            old = matching(rows,r,"legacy")
            text.append(f"| {r['C']}×{r['H']}×{r['W']} / {r['scale']} | {old['gpu_ms_median']:.4f} | {r['gpu_ms_median']:.4f} | {old['gpu_ms_median']/r['gpu_ms_median']:.2f}× |")
    text += ["", "训练提升约 1.3–2.7 倍；训练路径没有使用缺少 backward 的裸 CUDA 算子。", "",
             "## Nsight 定位与优化", "",
             ...
             "| alias_correction | 31.36 | 26.47 |", "| apply_correction | 28.80 | 22.05 |", "",
    ...
    for row in smoke["results"]:
        text.append(f"| {row['model']} | {row['dtype']} | {row['max_abs_vs_pytorch']:.3e} |")
    text += ["", "库不修改应用全局 TF32 开关。需要严格整网对照时，由调用方设置 `torch.backends.cudnn.allow_tf32=False` 和 `torch.backends.cuda.matmul.allow_tf32=False`。以上是集成验证，不是图像数据集 PSNR 测试。", "",
             "## 复现与文件", "",
    ...
    (ROOT / "gpu_validation.md").write_text("\n".join(text), encoding="utf-8")
