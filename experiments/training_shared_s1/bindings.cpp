...
using at::Tensor;
std::vector<Tensor> shared_s1_forward_cuda(const Tensor&,const Tensor&,const Tensor&);
std::vector<Tensor> shared_s1_backward_cuda(const Tensor&,const Tensor&,const Tensor&,const Tensor&,
    const Tensor&,bool,bool,bool);

namespace shared_s1_experiment {
...
public:
    static Tensor forward(torch::autograd::AutogradContext* ctx,Tensor y,Tensor k,Tensor lambda) {
        auto values=shared_s1_forward_cuda(y,k,lambda);
    ...
    static torch::autograd::variable_list backward(torch::autograd::AutogradContext* ctx,
        ...
        if(at::GradMode::is_enabled()) {
            ...
            auto values=shared_s1_backward_cuda(gradients[0],saved[0],saved[1],saved[3],saved[4],
...
TORCH_LIBRARY(shared_s1_experiment,m) {
    m.def("shared_s1_transfer(Tensor y, Tensor k, Tensor regularizer) -> Tensor",&shared_s1_experiment::transfer);
}
