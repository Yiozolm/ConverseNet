        ...
        for H,W in ((1,1),(1,2),(2,1),(3,2),(3,4),(4,6),(5,8),(5,7),(7,10),(2,3)):
            for KB,KC in ((1,1),(1,3),(2,1),(2,3)):
                data=(torch.randn(2,3,H,W//2+1,device='cuda',dtype=dtype).requires_grad_(),
                      torch.randn(2,3,2*H,W+1,device='cuda',dtype=dtype).requires_grad_(),
                      torch.randn(KB,KC,2*H,W+1,device='cuda',dtype=dtype).requires_grad_(),
                      torch.rand(1,3,1,1,device='cuda',dtype=real).add_(.2).requires_grad_())
                reference=spectral_reference(*data,H,W,2)
                old=ops['before']._training_spectral(*data,H,W,2)
                new=ops['fused']._training_spectral(*data,H,W,2)
                ...
                assert all(equality),(dtype,H,W,KB,KC)
...
def make_step(case,mode):
    ...
    def step():
        ...
        y=ops[mode].forward(x,p,k,b,2,1e-5)
...
def model():
    ...
    torch.manual_seed(221023)
    x=torch.rand(1,3,48,48,device='cuda');k=torch.rand(1,1,7,7,device='cuda');k=k/k.sum()
    ...
    for mode in ('before','fused'):
        with patch.object(torch.ops,'converse2d',ops[mode]):
            ...
            saved={}
            for i in range(3):
                opt.zero_grad(set_to_none=True);y=net(x,k,2);loss=(y-target).square().mean();loss.backward();opt.step()
    ...
    (HERE/'model.json').write_text(json.dumps(dict(shape='B1 HR96 s2 full pretrained USRNet, 3 Adam steps, fixed synthetic inputs',tensors=len(equal),bitwise_equal=sum(equal.values())),indent=2),encoding='utf-8')
