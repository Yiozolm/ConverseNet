...
def worker(route,metadata):
    ...
    up=torch.randn(4,32,128,160,device='cuda')*.001
    def step():
        with torch.cuda.nvtx.range('s2/'+route+'/forward'):
            prior=torch.nn.functional.interpolate(x,scale_factor=2,mode='nearest')
            y=selected.forward(x,prior,k,b,2,1e-5,'v7')
        with torch.cuda.nvtx.range('s2/'+route+'/backward'):
    ...
    Path(metadata).write_text(json.dumps(dict(status='complete',route=route,
        scope='B4/C32/LR64x80/s2, nearest prior, forward+x/kernel/bias VJPs; diagnostics only',
...
def launch(route,tool):
    ...
    bootstrap.write_text('import sys\n'+f'sys.path[:0] = {[str(Path(__file__).parent),str(ROOT/"test"),str(ROOT)]!r}\n'+
        'from profile_scale2 import worker\n'+f'worker({route!r},{str(metadata)!r})\n',encoding='utf-8')
    if tool=='nsys':
        ...
        cmd=[locate(tool),'--target-processes','all','--profile-from-start','off',
             '--kernel-name','regex:.*(adjoint_filter|fused_backward_s2).*',
