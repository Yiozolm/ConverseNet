...
CASES=[('tiny',1,3,3,5,1,1,3),('b1_c32',1,32,32,40,1,32,3),
       ...
       ('data_b1',1,64,48,48,1,64,7),('data_b4',4,64,48,48,4,64,7)]
def inputs(case):
    _,B,C,H,W,KB,KC,K=case
    rng=torch.Generator().manual_seed(220921)
    x=torch.randn(B,C,H,W,generator=rng)*.1
    p=torch.randn(B,C,H*2,W*2,generator=rng)*.1
...
def run_operators(ops):
    report=dict(identity=json.loads((HERE/'builds.json').read_text()),scope='Isolated s2 forward and all four VJPs; FP32 independent prior; no optimizer',cases=[])
    for case in CASES:
        cpu,up=inputs(case)
        ...
        def run(name):
            y=ops[name].forward(*data,2,1e-5)
...
def contracts(ops):
    ...
    for name in (n for n in ops if n!='before'):
        with patch.object(torch.ops,'converse2d',ops[name]),patch.object(fusion,'load_extension',lambda:None),patch.object(spatial,'load_extension',lambda:None):
            suite=unittest.TestSuite([unittest.defaultTestLoader.loadTestsFromModule(fusion),unittest.defaultTestLoader.loadTestsFromModule(spatial)])
