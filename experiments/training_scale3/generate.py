...
generic=generic.replace('launch_training_generic_', 'launch_training_scale3_')
prototypes='\n'.join(line.replace('launch_training_generic_','launch_training_scale3_')
...
'''+math_mode+generic+'\n}\n'
    (HERE/f'training_scale3_{mode}.cu').write_text(candidate,encoding='utf-8')
    texts=dict(before)
    ...
    gate='s==3' if mode=='const64' else 's==3 && p.numel()<=INT32_MAX-256 && H<=INT32_MAX/3 && W<=INT32_MAX/3'
    assert text.count(fwd)==text.count(bwd)==1
    text=text.replace(fwd,f'    else if({gate}) converse2d::scale3::launch_training_scale3_forward(y,p,k,l,out,q,d,H,W,s,stream);\n'+fwd)
    text=text.replace(bwd,f'    else if({gate}) converse2d::scale3::launch_training_scale3_backward(g,p,k,q,d,r,gd,gp,gk,H,W,s,need_p,need_k,no_broadcast,reduce_filter,stream);\n'+bwd)
    text='#include <ATen/ATen.h>\n#include <cuda_runtime.h>\n#include <cstdint>\nnamespace converse2d::scale3 {\n'+prototypes+'\n}\n'+text+'\n'+candidate
    texts['converse2d_training.cu']=text
    new_sources[mode]=texts
new_sources['before']=before
candidate=ROOT/'Converse2D/torch_converse2d/training/training_scale3.cu'
...
old=(HERE/'training_scale3_int32.cu').read_text(encoding='utf-8')
