...
$pythonPath = if ($env:CONVERSE_PYTHON) { $env:CONVERSE_PYTHON } else { Join-Path $repoRoot '.venv/Scripts/python.exe' }
...
$env:TEMP = Join-Path $repoRoot '.build/temp'
$env:TMP = $env:TEMP
New-Item -ItemType Directory -Force -Path $env:TEMP | Out-Null
& $pythonPath -u -X utf8 @TaskArgs
exit $LASTEXITCODE
